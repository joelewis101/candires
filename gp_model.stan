// gaussian process model with quadratic temporal kernal
// Time-varying effect of drug
...
parameters{
// population MIC parameters ----------------------------
real<lower=0> sigma_mic;    // population MIC sd
real mu_0;                  // population MIC mean (intercept)
//----- quadratic kernal bits --------------------------
...
for (i in 1:n_participants) {
  ...
  //print("t");
  //print(t[p:(p+N[i]-1)]);
  L_K = cholesky_decompose( gp_exp_quad_cov(t[p:(p+N[i]-1)], alpha,
  length_scale) + diag_matrix(rep_vector(1e-12, N[i]))); // jitter as per stan
  // docs
  //print("L_K");
  //print(L_K);
  f = L_K * eta[p:(p+N[i]-1)];
  ...
  //print("X");
  //print(X);
  y[p:(p+N[i] -1)] ~ normal(mu_0 + f + X*beta, sigma_mic);
  p = p + N[i];
}
