...
df_mod2 <-
  df_mod |>
  mutate(mic = if_else(is.na(mic), 99999, log(mic)))

# this will converge for albicans/fluconazole and glabrata/echinocandin
m <-
  msm(mic ~ t,
    subject = pid, data = df_mod2, qmatrix = q, covariates = ~covariate,
    hmodel = list(hmmNorm(mean = 1, sd = 1), hmmNorm(mean = 3, sd = 1)),
    censor = 99999,
    hconstraint = list(sd = c(1, 1))
  )

# how many 1st samples are na?
...
mod <- cmdstan_model(here("hmm.stan"))
