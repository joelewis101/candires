...
df_mod |>
  ...
  filter((index == 1 & is.na(sp_present)))
#

df_mod |>
  group_by(pid) |>
  filter(sum(!is.na(sp_present)) > 1) |>
  arrange(pid, t) |>
  mutate(index = seq_len(n())) |>
...
df_stan_bin |> group_by(pid,t,t_end) |> count() |> filter(n > 1)

# check for messed up segments

df_stan_bin |> group_by(pid) |> filter(t_end > lead(t))
df_stan_bin |> group_by(pid) |> filter(t_end < t)
df_stan_bin |> group_by(pid) |> filter(t_end != lead(t))

df_stan_bin


n_participants <- length(unique(df_stan_bin$pid))

n_obs_pairs_per_participant <- df_stan_bin |>
  group_by(pid) |>
  summarise(n = n()) |>
  pull(n)

t_start <- df_stan_bin$t
t_end <- df_stan_bin$t_end
state_start <- df_stan_bin$sp_present
state_end <- df_stan_bin$state_end
cov1 <- df_stan_bin$covariate
...
data_list <- list(
  ...
  t_start = t_start,
  t_end = t_end,
  state_start = if_else(is.na(state_start), -1, state_start),
  state_end = if_else(is.na(state_end), -1, state_end),
  covs = matrix(cov1),
  debug = 0
)
